 ...
 * 物理、エフェクト、UI、カメラなど、見た目・操作感に影響するすべての値。
 *
 * ⚠️ gameBalance.ts との関係性:
 *   - gameBalance.ts: テンポ・スコア・難易度に直結する核心値
 *   - tunables.ts: ここにあるので操作感やビジュアルは変わるが、難易度は変わらない
 *   - UPDATE_DISTANCES は gameBalance.ts と tunables.ts に両方存在する
 *     → 両ファイルの値は常に同期させること！
 *   - TEMPO_SPEED_BONUS も両方に存在（同期必須）
 *
 * 各セクション:
 *   PHYSICS         — プレイヤー物理（gameBalance.ts と同値）
 *   SHOOT           — シュートシステム
 *   THROW           — 投擲エンジン
 *   SPAWN           — ハザード・アイテムスポーン（細部設定）
 ...
 *   UI              — テキスト・HUD
 *   SCORE           — 距離スコア加算
 *   DIFFICULTY      — 難易度カーブ（ゲームバランス中核。gameBalance.ts と同期必須）
 *   BOSS            — ボス戦パラメータ
 *   RHYTHM_TUNING   — リズム系ジャンル用パラメータ
 *   STEALTH         — ステルス系ジャンル用パラメータ
 *   GENRE_PARAMS    — ジャンル分岐パラメータ設計用定数
 */
