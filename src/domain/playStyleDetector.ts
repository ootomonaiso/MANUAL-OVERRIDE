...
/** 各スタイルの判定に使用する最低閾値（rate は count / seconds＝1秒あたりの回数） */
const THRESHOLDS = {
  /** 攻撃的と判定する最低射撃レート（1秒あたりの射撃数） */
  aggressiveShotRate: 0.02,
  /** 防御的と判定する最低ジャンプレート（1秒あたり） */
  defensiveJumpRate: 0.015,
  /** 探索的と判定する最低移動レート（1秒あたり） */
  explorerMoveRate: 0.025,
  /** 混沌と判定する最低衝突レート（1秒あたり） */
  chaoticCollisionRate: 0.005,
  /** 消極的と判定する最高操作レート合計（1秒あたり） */
