...
import { isHazardous } from '../../../src/game/sideScroller'

/**
 * beat_hazard 反転判定のユニットテスト。
 *
 * isHazardous(beatHazardInverted, hasBeatHazard, isSafe) の
...
describe('isHazardous', () => {
  describe('反転ON + beat_hazard有効', () => {
    it('isSafe=true のハザードは危険（反転により safe → 危険）', () => {
      expect(isHazardous(true, true, true)).toBe(true)
    })

    it('isSafe=false のハザードは安全（反転により hazard → 安全）', () => {
      expect(isHazardous(true, true, false)).toBe(false)
    })
  })

  describe('反転OFF + beat_hazard有効', () => {
    it('isSafe=true のハザードは安全（従来通り）', () => {
      expect(isHazardous(false, true, true)).toBe(false)
    })

    it('isSafe=false のハザードは危険（従来通り）', () => {
      expect(isHazardous(false, true, false)).toBe(true)
    })
  })

  describe('反転ON + beat_hazard無効', () => {
    it('isSafe=true のハザードは安全（beat_hazard 無効なので反転も無効）', () => {
      expect(isHazardous(true, false, true)).toBe(false)
    })

    it('isSafe=false のハザードは危険（beat_hazard 無効なので従来通り）', () => {
      expect(isHazardous(true, false, false)).toBe(true)
    })
  })

  describe('反転OFF + beat_hazard無効', () => {
    it('isSafe=true のハザードは安全（通常時）', () => {
      expect(isHazardous(false, false, true)).toBe(false)
    })

    it('isSafe=false のハザードは危険（通常時）', () => {
      expect(isHazardous(false, false, false)).toBe(true)
    })
  })
})
