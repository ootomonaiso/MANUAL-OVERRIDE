...
describe('battleEngine: プレイヤーの初期化', () => {
  it('初期スキルは2種類からランダムに1つ選ばれ、スロット0に入る', () => {
    ...
    expect(a.actives[0]).toMatchObject({ level: 1, points: 0, cooldown: 0, slotIndex: 0 })
...
describe('battleEngine: 敵の生成', () => {
  ...
  it('定義から所持スキル・特性・行動パターンが移される', () => {
    ...
    expect(e.passives).toEqual([{ id: 'tough', level: 3 }])
    expect(e.actives).toEqual([{ id: 'bite', points: 0, level: 2, cooldown: 0, slotIndex: null }])
