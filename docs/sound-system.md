...
| レイヤー | ファイル | 責務 |
|---|---|---|
| 定義データ | `src/data/sfx/*.json`（76件） | 効果音1つ = ファイル1つ。何の音をどう鳴らすか |
...
`src/data/sfx/` には 76 件の JSON があるが、`SfxSound` のフックから再生されるのは 45 件。以下 6 件は**どこからも呼ばれていない**。

なお `battle_*` の 25 件はフック経由ではなく、rpg 戦闘のデータ（`src/data/rpg/battle-effects/*.json` の `sfx`、
`src/data/rpg/skills/*.json` の `sfx`）から **id 指定で** `soundManager.playSfx(id)` を呼んで鳴らしている
（[docs/genre/rpg/09-effects.md](genre/rpg/09-effects.md)）。`battle_skill_select` / `battle_draft_reroll` は
戦闘コンテンツのJSONではなく `BattleScreen.vue` 側の操作ハンドラから直接 `playSfx(id)` している点だけ異なる。
...
| ファイル | 検証内容 |
|---|---|
| [sfxLoader.test.ts](../tests/unit/sfxLoader.test.ts) | 76件の ID が過不足なく読み込まれるか、`id` とキーの整合性、各トラックの型・値域 |
