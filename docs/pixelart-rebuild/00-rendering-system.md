...
## 11. 改訂仕様（2026-08-24 / 実機確認による指摘を受けて追加）

S8 完了後、ユーザーが実機で確認したところ 2 件の品質問題が判明した。
**本節が当該箇所の最新の確定仕様であり、§4・§6 の記述に優先する。**

| # | 指摘 | 分類 |
|---|---|---|
| **R-01** | 円形の描画が不安定 | **仕様違反**（§4 の指定どおりに実装されていない） |
| **R-02** | 文字がかなり見づらい | **仕様不備**（§6 の方式に成立条件が欠けていた） |

### R-01: 円系プリミティブをセル整数演算へ

#### 現状の問題

§4 は `circle()` を**「ミッドポイント円アルゴリズムによるブロック円」**と規定していたが、
実装 [PixelCanvas.ts](../../src/game/render/PixelCanvas.ts) の `circle()` はスキャンラインの
平方根方式になっている。**これは仕様違反である。**
...
`rowH` も `halfW` もセルサイズの倍数ではない**端数**であり、
それを行ごとに `rect()` が独立してスナップしている。このため:

1. 行の高さが 1 セルになったり 2 セルになったりばらつく
2. 左右の端が行ごとに別々に丸められ、**上下・左右の対称性が崩れる**
3. `r` がわずかに変化すると `steps` が整数単位で跳び、**行構成が丸ごと組み替わる**

3 が「不安定」の主因である。半径が脈打つ演出（`pulse` 系）では
この組み替えが毎フレーム発生し、輪郭が沸き立って見える。

#### 確定仕様

**アンチエイリアスは採用しない。** 理由は 2 つ:

- 本仕様書 §1・§2 が定めた PixelArt 化の目的（滑らかな描画の排除）と正面から矛盾する
- **沸き立ちが直らない。** 形状が組み替わる現象はそのまま残り、輪郭がぼやけるだけである

原因は量子化の不整合でありジャギーではないため、**セル単位の整数演算**で解決する。

```
1. 半径をセル数へ量子化する:  R = max(1, round(r / size))
2. ミッドポイント円アルゴリズムを整数 R で実行し、行ごとのセル幅を得る
3. 各行はきっちり 1 セル高・整数セル幅の rect として出力する
4. 中心もセル境界へスナップし、対称性を保証する
```
...
| 性質 | 効果 |
|---|---|
| 決定性 | 同じ `R` なら常に同じ形。`r` の端数に依存しない |
| 対称性 | 上下・左右が鏡像になる（整数演算で 8 分円を反射するため） |
| 安定性 | `r` の変化に対し「組み替わる」のではなく **1 セルずつ太る/痩せる** |

`ellipse()` と `halfCircle()` も**同じ欠陥を持つ**。同一方針で改修する
（楕円はミッドポイント楕円、半円は円の上下いずれかの半分）。

#### 輪郭を柔らかく見せたい場合

アンチエイリアスではなく、ドット絵の作法に従う。いずれもグリッド上に留まる。

- **境界セルに 1 段暗い（明るい）色の縁を置く**（コンター）
- **境界を `dither()` で散らす**（既存 API で実現可能）

現時点ではどちらも必須としない。R-01 の整数化のみで安定性は解決するため、
**先に整数化だけを行い、実機確認のうえで必要なら追加する。**

### R-02: 文字の焼き込み解像度に下限を設ける

#### 現状の問題

`textScale: 3` は**フォントを 1/3 に縮めて焼き、3 倍に引き伸ばす**
（[PixelText.ts](../../src/game/render/PixelText.ts) の `_bake()`）。
つまり 16px のフォントは実質 **5px で描かれている。**

日本語の字形にはサイズ下限がある。目安は以下のとおり。
...
5px の漢字は「ドット化されている」のではなく**情報が失われている。**
§6 の方式自体は妥当だが、**倍率を固定した点が不備だった。**

#### なぜ `textScale` を下げるだけでは不十分か

`textScale` は単一の定数であり、フォントサイズによって結果が大きく異なる。

| 呼び出し側のフォント | `textScale: 3` での焼き込み | 判定 |
|---|---|---|
| 32px（見出し・GAME OVER 等） | 10px | ドット感があり読める |
| 12px（UI の小さい文字） | 4px | **完全に潰れる** |

`textScale` を一律に下げると、大きい文字のドット感まで失われ、
§2 で承認された「ドット絵風」という目的そのものが損なわれる。
...
```
minLegiblePx = PIXELART.textMinBakePx        // 新規追加する設定値
scale = clamp(1, PIXELART.textScale, fontSize / minLegiblePx)
```
...
`textMinBakePx` は `pixelart.json` に追加し、**コードに直書きしない**（§4 量子化の規則）。
初期値は漢字の下限に合わせて `11` を推奨するが、実機確認のうえで調整する。
`ConfigValidator` と `validate-json.mjs` の必須キー・範囲チェックにも追加すること（§9）。

なお `_cacheKey` は現在 `font` を含んでいるため（[PixelText.ts](../../src/game/render/PixelText.ts) の `_cacheKey()`）、
**キャッシュキーの変更は不要**である。フォントサイズが変われば別エントリになる。

### 検証方法

実機での目視が本来必要だが、本セッションの環境では実行できない（下記「懸念点」参照）。
オフスクリーン canvas に対する `getImageData` での実測は可能であり、最低限これを行う。

| 対象 | 実測項目 |
|---|---|
| R-01 | `r` を 0.1 刻みで動かし、**出力セル構成が単調に変化する**こと（組み替えが起きない） |
| R-01 | 中心軸で左右・上下のピクセル数が**一致する**こと（対称性） |
| R-02 | 12px / 16px / 32px の各フォントで、焼き込みサイズが `textMinBakePx` を**下回らない**こと |
...
- **未着手:** §11 の R-01（円のセル整数化）・R-02（文字の焼き込み下限）。
  2026-08-24 のユーザー実機確認による指摘。仕様は確定済み、**実装は次フェーズ（S9）で行う。**
