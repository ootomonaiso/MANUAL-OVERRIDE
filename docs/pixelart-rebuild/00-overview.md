...
### S9: 実機確認と監査2本による品質改修（2026-08-24、完了）

S8 完了後にコミット・push し、**ユーザーの実機確認**および監査2本
（[claude-audit-report-03.md](claude-audit-report-03.md) / [audit-report-04.md](audit-report-04.md)）で
新たな問題が判明した。仕様は [00-rendering-system.md §11](00-rendering-system.md) に確定し、**実装済み。**

| # | 指摘 | 対処 | 検証 |
|---|---|---|---|
| **R-01** | 円形の描画が不安定 | 円系プリミティブをセル整数演算へ（**直径を量子化**）。`ellipse`/`halfCircle`/`_annulus` も同様。**アンチエイリアスは採用しない**（PixelArt 化の目的と矛盾し、かつ沸き立ちが直らないため） | 半径を動かしても既存行が動かないことを実測 |
| **R-02** | 文字がかなり見づらい | 焼き込み後サイズに下限（`textMinBakePx = 11`）を設定。**倍率は整数に限る** | 全フォントで焼き込み ≧ 11px |
| **C-01** | `ellipse` の高さが1.5倍に膨張（影 9 箇所） | 同上（直径量子化） | ry=4 の影が 12px → **8px** |
| **C-02** | `circle` が左右非対称 | 幅を直径と偶奇を揃えて中央寄せ | 非対称px **128 → 0** |
| **C-03** | `arcBlocks` の重複描画で半透明時に輝度ムラ | `arcBlocks`/`line` で同一セルの二重描画を抑止 | アルファ値 **128・192 → 128 のみ** |
| **C-04** | 未解決スロットで警告が出ない | DEV で 1 回だけ警告 | — |
| **C-05** | `line` が仕様の Bresenham ではない | **表示に欠陥がないため仕様書の記述を実装に合わせた** | 途切れ 0・セル数一定を実測 |
| **C-06/07** | `block()` のハードコード / コメント誤り | `blockShadeAmount` を JSON 化・`_EPSILON` 定数化・コメント修正 | — |
| **C-08** | 描画中核に単体テストが皆無 | `tests/unit/game/render/` に **25 件**追加 | 旧実装に戻すと確実に落ちることを確認済み |

**D9（日本語の判読性）は「後退条件に該当」と判定された**が、
後退（ドット化の取りやめ）ではなく R-02 の方式で解決した。

### 第4回監査の指摘のうちスコープ外としたもの

`audit-report-04.md` は G-01〜G-07・J-01〜J-04・V-01/02・T-01・UX-01〜03・REG-01 も指摘している。
**これらは本ブランチでは修正していない。** 判定根拠は
[00-rendering-system.md §11.8](00-rendering-system.md) に記録した。

- `CLAUDE_OWNER.md` が「見た目（描画）以外の変更」を明確に禁止している
- **全項目が PixelArt 変換前（`c40c1a7`）から存在する**ことを確認済み（本ブランチ由来ではない）
