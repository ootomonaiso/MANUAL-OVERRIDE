...
|---|---|---|---|---|
| stg | 1500 | 500 | 0.00025 | 密集した敵（`arena` 相当） |
| aerial_stg | 1500 | 500 | 0.00025 | `stg` と同水準（射撃インフラ共有の近縁ジャンル） |
...
> `stg` は元々 1800/600/0.0002（同系統の `arena`/`bullet_hell` より疎）、`aerial_stg` は
> `spawnDensity` 自体が未設定でグローバル既定値（2400/800/0.00015、`runner` 並みの疎さ）に
> フォールバックしていた。射撃・キル数をスコアの主要因とするジャンルとしては疎すぎ、
> 敵を倒しづらくキル数が伸びない一因になっていたため密度を引き上げた。
