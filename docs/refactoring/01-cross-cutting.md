...
### 2-2.【med】ブランチ専用の指示ファイルが肥大化・重複している

- `CLAUDE_OWNER.md`（182行）— 「`CLAUDE.md` より優先。適用範囲は本ブランチ限定」を宣言。
  当初のタスク定義に、後付けの追加要件2件（敵グループ/難易度スケーリング、スキルポイント制度）が追記されている。
  **このファイル自体が「ツギハギの出典」であり、実装が完了した今は履歴的文書。**
- `CLAUDE_TASKS.md`（1024行）— 7フェーズ分の作業ログが1ファイルに連結。リポジトリ直下。

**修正案（Phase 9）:** どちらもリポジトリ直下の一級ファイルとして残すべきか、
`docs/genre/rpg/` 配下の履歴文書へ移すかを判断する。**削除ではなく移設・分割の判断であり、ユーザー確認が必要。**
本計画では「判断待ち」として記録するに留める。
