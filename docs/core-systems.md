...
- `tutorial`: チュートリアルプレイ開始。ver1.0 の横スクロールをそのまま遊ぶ。
- `updating`: 説明書更新フェーズ。2 択パネルを表示。
- `playing`: 通常プレイフェーズ。
- `genreLocked`: ジャンル確定フェーズ。選択の蓄積によって特定ジャンルへ収束。
- `throwing`: 投擲フェーズ。説明書UIをドラッグして投げる。
- `ending`: エンディングフェーズ。最終スコア表示 + ジャンル別メッセージ。
